...
const MainPage = ({ cards, toggleChecked, saveSelected, saveAll }) => {
  ...
  return (
    <div>
      ...
      <div >
        <select value={filterOption} onChange={handleFilterChange} id="all">
          <option value="" >All Arbitrage Opportunities</option>
      ...
      <button className="save-sel" onClick={saveSelected}>Save Selected</button>
      <button className="save-all"onClick={saveAll}>Save All</button>
      <Link className="show" to="/saved">Show Saved</Link>
    </div>
  );
};
