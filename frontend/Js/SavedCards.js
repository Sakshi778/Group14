...
const SavedCards = ({ savedCards, savedChecked, setSavedCards, setSavedChecked }) => {
  ...
  const handleRemoveSelected = () => {
    const updatedSavedCards = savedCards.filter((card) => !selectedCards.includes(card.id));
    const updatedSavedChecked = savedChecked.filter((id) => !selectedCards.includes(id));
    setSavedCards(updatedSavedCards);
    setSavedChecked(updatedSavedChecked);
    setSelectedCards([]);
  };
  ...
  return (
    ...
    <div className="card-grid">
      {filteredCards.map((card) => (
        <div key={card.id} className="card">
          <div className="card-content">
            <h3>{card.title}</h3>
            <p><b>BSE: </b>{card.bse}</p>
            <p><b>NSE: </b>{card.nse}</p>
            <p><b>Profit: </b>{card.profit}</p>
            <input
              type="checkbox"
      ...
      <div>
        <button className="remove-sel"onClick={handleRemoveSelected}>Remove Selected</button>
        <button className="remove-all"onClick={handleRemoveAll}>Remove All</button>
      </div>
      <Link className="go" to="/">Go Back</Link>
    </div>
  );
};
